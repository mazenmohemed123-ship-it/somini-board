...
> **🌐 نسخة حيّة:** https://somini-board.web.app
> لوحة مالك المنصة (superAdmin) على **`/admin`** — تدير كل الشركات المستأجَرة من مكان واحد.

> **حالة المشروع:** منشور ويعمل على Firebase (Firestore + Functions 2nd gen +
> Hosting + Realtime DB). الأجزاء الأمنية الأساسية (Security Rules، Cloud Functions،
> عزل الـ Tenants) مكتملة بكود حقيقي. بعض الخطوات تتطلب تفعيلًا يدويًا من Console أو
> مفاتيح حقيقية (Paymob، App Check) — انظر "ما يتطلب إعدادًا يدويًا".

## لوحة مالك المنصة (superAdmin) — `/admin`
لوحة خاصة بصاحب المنصة فقط (محميّة بـ claim الدور `superAdmin`)، لا يراها أي مستخدم آخر:
- **`/admin`** — بطاقات إحصائية (شركات نشطة، اشتراكات شهرية، انتخابات جارية، إجمالي
  الموظفين)، رسم بياني لنمو الشركات (Chart.js)، وجدول بأحدث الشركات.
- **`/admin/tenants`** — كل الشركات مع عدد الموظفين/الانتخابات والخطة وحالة الاشتراك،
  وزر تعطيل/تفعيل (يُعطّل تسجيل الدخول عبر Identity Platform أيضًا).
- **`/admin/tenants/[tenantId]`** — تفاصيل شركة + إحصائياتها + تغيير الخطة/تمديد الاشتراك.
- **`/admin/subscriptions`** — كل الاشتراكات مع فلترة (تنتهي قريبًا / منتهية).

مدعومة بـ ٦ Cloud Functions للـ superAdmin (`getSuperAdminStats`، `listAllTenants`،
`getTenantDetails`، `toggleTenantStatus`، `updateSubscription`، `listAllSubscriptions`)
— كلها تتحقق من الدور `superAdmin` على السيرفر، والتجميع عبر كل الشركات يتم بـ Admin SDK.
